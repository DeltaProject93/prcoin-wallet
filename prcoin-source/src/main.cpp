...
#if defined(NDEBUG)
# error "Prcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb2b1579259e475c9e75aad3e8a71e96d824ecd13d4b6c3c384b90a711d669fc9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Prcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Prcoin Signed Message:\n";
...
{
    // Prcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Prcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Prcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1474388107, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0493fc5077ec099b5d4f263b87e7097c0d2a0d79ab66e10316148adb1d7d977c353dd8cfb9ed55daad9250523bf381ad1f5a410cd87bc22b815c6440a1739dfc9d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1474388107;
        ...
        assert(block.hashMerkleRoot == uint256("0x041dccf83a01d0aa6dd481af02c5aa9d951a4b1a635696da2fbb44e7d8c091c0"));
...
unsigned char pchMessageStart[4] = { 0xef, 0xc1, 0x40, 0xba }; // Prcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PrcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PrcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PrcoinMiner(CWallet *pwallet)
{
    printf("PrcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("prcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PrcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PrcoinMiner, pwallet));
}
